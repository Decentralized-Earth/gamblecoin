...
static const char *strDNSSeed[][2] = {
    {"gamblecoin.com", "5.189.168.61"},
    {"gamblecoin.org", "193.37.152.248"},
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Gamblecoin is probably already running."), addrBind.ToString().c_str());
        else
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_MINER] > 0) printf("ThreadGamblecoinMiner still running\n");
